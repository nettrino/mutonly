...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    if (Options.MutOnly && TotalNumberOfRuns > Options.MaxNumberOfRuns)
      break;
    else if (TotalNumberOfRuns >= Options.MaxNumberOfRuns)
      break;
    size_t NewSize = 0;
    ...
    Size = NewSize;
    if (Options.MutOnly) {
      // Corpus.AddToCorpus({CurrentUnitData, CurrentUnitData + Size}, 1,
                         // [>MayDeleteFile=<]true);
      WriteToOutputCorpus({CurrentUnitData, CurrentUnitData + Size});
      TotalNumberOfRuns++;
      continue;
    }
    if (i == 0)
